#!/usr/bin/env python3
"""
Simple original bot without broadcast functionality
"""
...
async def stocks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    logger.info(f"STOCKS from {user.id}")
    ...
    await update.message.reply_text(stocks_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)

async def hookahs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /hookahs command"""
    user = update.effective_user
    logger.info(f"HOOKAHS from {user.id}")
    ...
    await update.message.reply_text(hookahs_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
...
def main():
    ...
    logger.info("Starting simple original bot...")
